 ...
 * Modificado por PipaGerardo para compilarlo en puro C                 *
...
DLLEXP kailleraGetVersion( char *version ) {
    return (INT_PTR)strcpy( version, myVersion );
}
...
DLLEXP kailleraInit() {
    ...
    //Get Contents
        ...
        //Server Tab
			if      (strncmp(temp, "ServerTab=0", 11) == 0) lastTabServer = 0;
			else if (strncmp(temp, "ServerTab=1", 11) == 0) lastTabServer = 1;
			else if (strncmp(temp, "ServerTab=2", 11) == 0) lastTabServer = 2;
			else if (strncmp(temp, "ServerTab=3", 11) == 0) lastTabServer = 3;
			else if (strncmp(temp, "ServerTab=4", 11) == 0) lastTabServer = 4;
        //Show Chatroom First?
			else if (strncmp(temp, "ShowChatroomFirst=1", 19) == 0) showChatroomFirst = true;
			else if (strncmp(temp, "ShowChatroomFirst=0", 19) == 0) showChatroomFirst = false;
        //chkJoinChat
			else if (strncmp(temp, "Join/Left=0", 11) == 0) joinChatValue = 0;
            else if (strncmp(temp, "Join/Left=1", 11) == 0) joinChatValue = 1;
        //chkJoinChatGame
			else if (strncmp(temp, "Join/Left_Game=0", 16) == 0) joinChatGameValue = 0;
            else if (strncmp(temp, "Join/Left_Game=1", 16) == 0) joinChatGameValue = 1;
        //chkBeep
			else if (strncmp(temp, "Beep=0", 6) == 0) beepValue = 0;
            else if (strncmp(temp, "Beep=1", 6) == 0) beepValue = 1;
        //chkDrop
			else if (strncmp(temp, "Drop=0", 6) == 0) dropValue = 0;
			else if (strncmp(temp, "Drop=1", 6) == 0) dropValue = 1;
        //chkDrop
			else if (strncmp(temp, "Join_Double_Click=0", 19) == 0) joinDblValue = 0;
			else if (strncmp(temp, "Join_Double_Click=1", 19) == 0) joinDblValue = 1;
        //chkShowError
			else if (strncmp(temp, "Join_Error=0", 12) == 0) errorValue = 0;
            else if (strncmp(temp, "Join_Error=1", 12) == 0) errorValue = 1;
        //EmulinkerSF MaxUsers
			else if (strncmp(temp, "EmuSF_MaxUsers=", 15) == 0) strcpy(maxUsersG, &temp[15]);
        //EmulinkerSF MaxPing
			else if (strncmp(temp, "EmuSF_MaxPing=", 14) == 0) strcpy(maxPingG, &temp[14]);
        //EmulinkerSF FakeP2P
            else if (strncmp(temp, "EmuSF_FakeP2P=1", 15) == 0) fakeP2PValue = 1;
            else if (strncmp(temp, "EmuSF_FakeP2P=0", 15) == 0) fakeP2PValue = 0;
        //EmulinkerSF REmulator
			else if (strncmp(temp, "EmuSF_Emulator=1", 16) == 0) emuResValue = 1;
            else if (strncmp(temp, "EmuSF_Emulator=0", 16) == 0) emuResValue = 0;
        //EmulinkerSF RConnection Type
            else if (strncmp(temp, "EmuSF_Connection=1", 18) == 0) connResValue = 1;
            else if (strncmp(temp, "EmuSF_Connection=0", 18) == 0) connResValue = 0;
        //EmulinkerSF REmulator
			else if (strncmp(temp, "Userlist_Column=0", 17) == 0) lstUserlistColumn = 0;
            else if (strncmp(temp, "Userlist_Column=1", 17) == 0) lstUserlistColumn = 1;
            else if (strncmp(temp, "Userlist_Column=2", 17) == 0) lstUserlistColumn = 2;
            else if (strncmp(temp, "Userlist_Column=3", 17) == 0) lstUserlistColumn = 3;
            else if (strncmp(temp, "Userlist_Column=4", 17) == 0) lstUserlistColumn = 4;
        ...
        //chkEmulinkerSF
			else if (strncmp(temp, "Use_EmuSF=0", 11) == 0) emulinkerSFValue = 0;
            else if (strncmp(temp, "Use_EmuSF=1", 11) == 0) emulinkerSFValue = 1;
        //chkKeepGameChatLogs
            else if (strncmp(temp, "Keep_Gameroom_Logs=0", 20) == 0) gameChatLogValue = 0;
            else if (strncmp(temp, "Keep_Gameroom_Logs=1", 20) == 0) gameChatLogValue = 1;
        //chkKeepChatLogs
			else if (strncmp(temp, "Keep_Chatroom_Logs=0", 20) == 0) chatLogValue = 0;
            else if (strncmp(temp, "Keep_Chatroom_Logs=1", 20) == 0) chatLogValue = 1;
        //chkBlink
			else if (strncmp(temp, "Blink=0", 7) == 0) blinkValue = 0;
            else if (strncmp(temp, "Blink=1", 7) == 0) blinkValue = 1;
        //chkUseScreenChat
			else if (strncmp(temp, "Use_Screen_Chat=0", 17) == 0) useScreenChatValue = 0;
            else if (strncmp(temp, "Use_Screen_Chat=1", 17) == 0) useScreenChatValue = 1;
        //chkCreate
			else if (strncmp(temp, "Create/Close=0", 14) == 0) createValue = 0;
            else if (strncmp(temp, "Create/Close=1", 14) == 0) createValue = 1;
        //ServerIP
			else if (strncmp(temp, "ServerIP=", 9) == 0) strcpy(serverIP, &temp[9]); //ServerIP=
        //Nick
        ...
        //Connection
                ...
                if      (strncmp(temp, "Connection=1", 12) == 0) connectionType = lan;
                else if (strncmp(temp, "Connection=2", 12) == 0) connectionType = excellent;
                else if (strncmp(temp, "Connection=3", 12) == 0) connectionType = good;
                else if (strncmp(temp, "Connection=4", 12) == 0) connectionType = average;
                else if (strncmp(temp, "Connection=5", 12) == 0) connectionType = low;
                else if (strncmp(temp, "Connection=6", 12) == 0) connectionType = bad;
		...
		//LastGameToPlay1
			else if (strncmp(temp, "LastGameToPlay1=", 16) == 0) strcpy(lastGameToPlay1, &temp[16]);
        //LastGameToPlay2
			else if (strncmp(temp, "LastGameToPlay2=", 16) == 0) strcpy(lastGameToPlay2, &temp[16]);
        //LastGameToPlay3
			else if (strncmp(temp, "LastGameToPlay3=", 16) == 0) strcpy(lastGameToPlay3, &temp[16]);
		...
		case SC_SUPRARECVWAITING:{
			switch(WSAGETSELECTEVENT(lParam)){
				case FD_CONNECT:{
					...
					strcat(dataToBeSent,"\r\n\r\n");
					send(mySocketWaiting, dataToBeSent, strlen(dataToBeSent) + 1, 0);
					return 0;
				}
				case FD_READ:{
					bytesRecvWaiting = recv(mySocketWaiting, myBuffWaiting, sizeof(myBuffWaiting), 0);
					parseWaitingGames();
					return 0;
				}
			}
		...
		case SC_SUPRARECV3D:{
			switch(WSAGETSELECTEVENT(lParam)){
				case FD_CONNECT:{
					...
					strcat(dataToBeSent, anti3DServerList.host);
					strcat(dataToBeSent,"\r\n\r\n");
					send(mySocket3D, dataToBeSent, strlen(dataToBeSent) + 1, 0);
					return 0;
				}
				case FD_READ:{
					bytesRecvServerList3D = recv(mySocket3D, myBuffServerList3D, sizeof(myBuffServerList3D), 0);
					parseServerList3D();
					return 0;
				}
			}
		...
		case SC_SUPRARECVK:{
			switch(WSAGETSELECTEVENT(lParam)){
				case FD_CONNECT:{
					...
					strcat(dataToBeSent,"\r\n\r\n");
					send(mySocketK, dataToBeSent, strlen(dataToBeSent) + 1, 0);
					return 0;
				}
				case FD_READ:{
					bytesRecvServerListK = recv(mySocketK, myBuffServerListK, sizeof(myBuffServerListK), 0);
					parseServerListK();
					return 0;
				}
			}
...
DWORD WINAPI pingKailleraServers(LPVOID lpParam){
	...
	for(i = i; i < total; i++){
		...
		if(sError == false){
			...
			sendto( mySocketK, "PING\0",  5, 0, (struct sockaddr *) &socketInfoK, sizeof(socketInfoK) );

			//Check the socket
			int timeout = GetTickCount();
			buff = 0;
			while(buff < 1 && GetTickCount() - timeout <= 1000){
				buff = recv(mySocketK, myBuff, sizeof(myBuff), 0);
				if(buff > 1){
					if(strncmp(myBuff, "PONG", 4) == 0){
						sprintf(strPing, "%ims", (int)(GetTickCount() - timeout) );
						b.iItem = i;
						b.iSubItem = 2;
						b.pszText = strPing;
						SendMessage(lstServerListK, LVM_SETITEMTEXT, (WPARAM)i, (LPARAM)&b);
			...
			if(here == false){
				sprintf(strPing, "%i",(int)(GetTickCount() - timeout));
				b.iItem = i;
				b.iSubItem = 2;
				b.pszText = ">1s";
				SendMessage(lstServerListK, LVM_SETITEMTEXT, (WPARAM)i, (LPARAM)&b);
...
DWORD WINAPI ping3DServers(LPVOID lpParam){
	...
	for(i = i; i < total; i++){
		...
		if(sError == false){
			...
			sendto(mySocket3D, "PING\0",  5, 0, (struct sockaddr *) &socketInfo3D, sizeof(socketInfo3D));

			//Check the socket
			int timeout = GetTickCount();
			buff = 0;
			while(buff < 1 && GetTickCount() - timeout <= 1000){
				buff = recv(mySocket3D, myBuff, sizeof(myBuff), 0);
				if(buff > 1){
					if(strncmp(myBuff, "PONG", 4) == 0){
						sprintf(strPing, "%ims", (int)(GetTickCount() - timeout));
						b.iItem = i;
...
void popupMenu(char num){
	...
	//User List
		...
		//PM
			...
			// if( strlen(fPM) == NULL ) return;
			if( strlen(fPM) == 0 ) return;
...
int saveConfig(){
	...
	// config << "Quit=" << temp << "\n";
    fputs( "Quit=", config ); fputs( temp, config ); fputc( '\n', config );
	...
	// config << "xPos=" << q.left << "\n";
	ltoa( q.left, temp, 10 ); fputs( "xPos=", config ); fputs( temp, config ); fputc( '\n', config );
	// config << "yPos=" << q.top << "\n";
	ltoa( q.top, temp, 10 );  fputs( "yPos=", config ); fputs( temp, config ); fputc( '\n', config );

	//LastGameToPlay1
	// config << "LastGameToPlay1=" << lastGameToPlay1 << "\n";
	fputs( "LastGameToPlay1=", config ); fputs( lastGameToPlay1, config ); fputc( '\n', config );

	//LastGameToPlay2
	// config << "LastGameToPlay2=" << lastGameToPlay2 << "\n";
	fputs( "LastGameToPlay2=", config ); fputs( lastGameToPlay2, config ); fputc( '\n', config );

	//LastGameToPlay3
	// config << "LastGameToPlay3=" << lastGameToPlay3 << "\n";
	fputs( "LastGameToPlay3=", config ); fputs( lastGameToPlay3, config ); fputc( '\n', config );

	//UserListColumn
	// config << "Userlist_Column=" << lstUserlistColumn << "\n";
	itoa( lstUserlistColumn, temp, 10 ); fputs(  "Userlist_Column=", config ); fputs( temp, config ); fputc( '\n', config );

	// config << "Userlist_Column_Order=" << userlistSwitch << "\n";
	itoa( userlistSwitch, temp, 2 ); fputs( "Userlist_Column_Order=", config ); fputs( temp, config ); fputc( '\n', config );

	int v;
	//chkKeepGameChatLogs
	v = SendMessage(chkKeepGameChatLogs, BM_GETCHECK, 0, 0);
	// config << "Keep_Gameroom_Logs=" << v << "\n";
	itoa( v, temp, 10 ); fputs( "Keep_Gameroom_Logs=", config ); fputs( temp, config ); fputc( '\n', config );

	//chkKeepChatLogs
	v = SendMessage(chkKeepChatLogs, BM_GETCHECK, 0, 0);
	// config << "Keep_Chatroom_Logs=" << v << "\n";
    itoa( v, temp, 10 ); fputs( "Keep_Chatroom_Logs=", config ); fputs( temp, config ); fputc( '\n', config );

	//chkJoinChat
	v = SendMessage(chkJoinChat, BM_GETCHECK, 0, 0);
	// config << "Join/Left=" << v << "\n";
    itoa( v, temp, 10 ); fputs( "Join/Left=", config ); fputs( temp, config ); fputc( '\n', config );

	//chkCreate
	v = SendMessage(chkCreate, BM_GETCHECK, 0, 0);
	// config << "Create/Close=" << v << "\n";
    itoa( v, temp, 10 ); fputs( "Create/Close=", config ); fputs( temp, config ); fputc( '\n', config );

	//chkEmulinkerSF
	v = SendMessage(chkEmulinkerSF, BM_GETCHECK, 0, 0);
	// config << "Use_EmuSF=" << v << "\n";
    itoa( v, temp, 10 ); fputs( "Use_EmuSF=", config ); fputs( temp, config ); fputc( '\n', config );

	//chkBlink
	v = SendMessage(chkBlink, BM_GETCHECK, 0, 0);
	// config << "Blink=" << v << "\n";
    itoa( v, temp, 10 ); fputs( "Blink=", config ); fputs( temp, config ); fputc( '\n', config );

	//chkUseScreenChat
	v = SendMessage(chkUseScreenChat, BM_GETCHECK, 0, 0);
	// config << "Use_Screen_Chat=" << v << "\n";
    itoa( v, temp, 10 ); fputs( "Use_Screen_Chat=", config ); fputs( temp, config ); fputc( '\n', config );

	//chkDrop
	v = SendMessage(chkDrop, BM_GETCHECK, 0, 0);
	// config << "Drop=" << v << "\n";
    itoa( v, temp, 10 ); fputs( "Drop=", config ); fputs( temp, config ); fputc( '\n', config );

	//chkBeep
	v = SendMessage(chkBeep, BM_GETCHECK, 0, 0);
	// config << "Beep=" << v << "\n";
    itoa( v, temp, 10 ); fputs( "Beep=", config ); fputs( temp, config ); fputc( '\n', config );

	//chkJoinDbl
	v = SendMessage(chkJoinDbl, BM_GETCHECK, 0, 0);
	// config << "Join_Double_Click=" << v << "\n";
    itoa( v, temp, 10 ); fputs( "Join_Double_Click=", config ); fputs( temp, config ); fputc( '\n', config );

	//chkUseCache
	v = SendMessage(chkUseCache, BM_GETCHECK, 0, 0);
	// config << "Cache=" << v << "\n";
    itoa( v, temp, 10 ); fputs( "Cache=", config ); fputs( temp, config ); fputc( '\n', config );
    ...
    itoa( v, temp, 10 ); fputs( "EmuSF_Emulator=", config ); fputs( temp, config ); fputc( '\n', config );

	//EmuSF_Connection Type
	v = SendMessage(chkConnRes, BM_GETCHECK, 0, 0);
	// config << "EmuSF_Connection=" << v << "\n";
	itoa( v, temp, 10 ); fputs( "EmuSF_Connection=", config ); fputs( temp, config ); fputc( '\n', config );
	...
	fputs( "EmuSF_MaxUsers=", config ); fputs( maxUsersG, config ); fputc( '\n', config );
	...
	fputs( "EmuSF_MaxPing=", config ); fputs( maxPingG, config ); fputc( '\n', config );

	//chkFakeP2P
	v = SendMessage(chkFakeP2P, BM_GETCHECK, 0, 0);
	// config << "EmuSF_FakeP2P=" << v << "\n";
	itoa( v, temp, 10 ); fputs( "EmuSF_FakeP2P=", config ); fputs( temp, config ); fputc( '\n', config );

	//chkJoinChatGame
	v = SendMessage(chkJoinChatGame, BM_GETCHECK, 0, 0);
	// config << "Join/Left_Game=" << v << "\n";
	itoa( v, temp, 10 ); fputs( "Join/Left_Game=", config ); fputs( temp, config ); fputc( '\n', config );

	//lastTabServer
	v = SendMessage(sTab, TCM_GETCURSEL, 0, 0);
	// config << "ServerTab=" << v << "\n";
	itoa( v, temp, 10 ); fputs( "ServerTab=", config ); fputs( temp, config ); fputc( '\n', config );

	//lastTabServer
	// config << "ShowChatroomFirst=" << 0 << "\n";
	fputs( "ShowChatroomFirst=0", config ); fputc( '\n', config );
	...
	for(v = 0; v < t; v++){
		...
		fputs( "Recent_IP", config ); fputs( strI, config ); fputs( "=", config ); fputs( temp, config ); fputc( '\n', config );

		b.iSubItem = 1;
		b.pszText = temp;
		SendMessage(lstRecentList, LVM_GETITEM, 0, (LPARAM) &b);
		// config << "Recent_Name" << strI << "=" << temp << endl;
        fputs( "Recent_Name", config ); fputs( strI, config ); fputs( "=", config ); fputs( temp, config ); fputc( '\n', config );

		b.iSubItem = 2;
		b.pszText = temp;
		SendMessage(lstRecentList, LVM_GETITEM, 0, (LPARAM) &b);
		// config << "Recent_Location" << strI << "=" << temp << endl;
		fputs( "Recent_Location", config ); fputs( strI, config ); fputs( "=", config ); fputs( temp, config ); fputc( '\n', config );
	...
	for(v = 0; v < t; v++){
		...
		fputs( "Favorite_IP", config ); fputs( strI, config ); fputs( "=", config ); fputs( temp, config ); fputc( '\n', config );

		b.iSubItem = 1;
		b.pszText = temp;
		SendMessage(lstFavoriteList, LVM_GETITEM, 0, (LPARAM) &b);
		// config << "Favorite_Name" << strI << "=" << temp << endl;
        fputs( "Favorite_Name", config ); fputs( strI, config ); fputs( "=", config ); fputs( temp, config ); fputc( '\n', config );

		b.iSubItem = 2;
		b.pszText = temp;
		SendMessage(lstFavoriteList, LVM_GETITEM, 0, (LPARAM) &b);
		// config << "Favorite_Location" << strI << "=" << temp << endl;
        fputs( "Favorite_Location", config ); fputs( strI, config ); fputs( "=", config ); fputs( temp, config ); fputc( '\n', config );

		b.iSubItem = 3;
		b.pszText = temp;
		SendMessage(lstFavoriteList, LVM_GETITEM, 0, (LPARAM) &b);
		// config << "Favorite_Comments" << strI << "=" << temp << endl;
		fputs( "Favorite_Comments", config ); fputs( strI, config ); fputs( "=", config ); fputs( temp, config ); fputc( '\n', config );
	...
	for(v = 1; v <= awayMessageCount; v++){
		if(awayMessages[v].save == true){
			...
			itoa( v, temp, 10 );
            fputs( "Away_Subject", config );
            fputs( temp, config );
            fputs( "=", config );
			fputs( awayMessages[v].subject, config );
			fputc( '\n', config );

			// config << "Away_Message" << v << "=" << awayMessages[v].message << endl;
            fputs( "Away_Message", config );
            fputs( temp, config );
            fputs( "=", config );
			fputs( awayMessages[v].message, config );
			fputc( '\n', config );
...
DWORD WINAPI recvLoop(LPVOID lpParam){
	...
	for(; ;){
		...
		mainBytesRecv = recv(mySocket, myBuff[myBuffCount].myBuff, sizeof(myBuff[myBuffCount].myBuff), 0);
...
void exitPing3DThread() {
	if (pinging3D) {
		...
		TerminateThread((HANDLE)ping3DServers, 0);
...
void userQuitRequest(){
	if (loggedIn == true) {
		...
		dataToBeSent[0] = (char)0x00;
		dataToBeSent[1] = (char)0xFF;
		dataToBeSent[2] = (char)0xFF;
...
void createGameRequest(){
	...
	dataToBeSent[1 + gameLength + 1] = (char)0x00;
	dataToBeSent[1 + gameLength + 2] = (char)0xFF;
	dataToBeSent[1 + gameLength + 3] = (char)0xFF;
	dataToBeSent[1 + gameLength + 4] = (char)0xFF;
	dataToBeSent[1 + gameLength + 5] = (char)0xFF;
...
void joinGameRequest(){
	...
	dataToBeSent[8] = 0x00;
	dataToBeSent[9] = 0x00;
	dataToBeSent[10] = (char)0xFF;
	dataToBeSent[11] = (char)0xFF;
...
void playerInformation(unsigned short position, int slot){
	...
	//For Each User
    for(w = 0; (unsigned long)w < numOfUsers; w++){
        //Nick
		strSize = strlen(&myBuff[slot].myBuff[i]);
		strncpy(nick, &myBuff[slot].myBuff[i], strSize + 1);
        i = i + strSize + 1;
...
void closeGameNotification(unsigned short position, int slot){
	...
	if(gameID == (unsigned long)myGameID){
		supraCleanup(1, 0);
		showGameroom(false);
		imOwner = false;
	}
...
void startGameRequest(){
	...
	dataToBeSent[1] = (char)0xFF;
	dataToBeSent[2] = (char)0xFF;
	dataToBeSent[3] = (char)0xFF;
	dataToBeSent[4] = (char)0xFF;
