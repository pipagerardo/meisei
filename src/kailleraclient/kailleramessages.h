...
struct AWAY_MESSAGES awayMessages[10];
...
struct sockaddr_in socketInfoWaiting;
...
struct SERVER_LIST kailleraServerList;
...
struct USER_LIST_COMMANDS userlistCommands[60];
...
struct INCOMING_BUFFER myBuff[MAX_INCOMING_BUFFER];
int myBuffCount = -1;
...
struct Packets myPackets[MAX_PACKETS];
...
#define outCacheSize    (bufferSize * 6)
#define inCacheSize     (maxPlayers * outCacheSize)
#define lastInCachePos  ((cacheSize - 1) * inCacheSize)
#define lastOutCachePos ((cacheSize - 1) * outCacheSize)
